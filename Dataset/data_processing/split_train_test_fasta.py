...
def split_train_test(in_fasta, out_fasta_train, out_fasta_test, test_size):
    ...
    test_odict = OrderedDict(
        (key, seq_odict[key]) for key in test_keys if key in seq_odict
    )
    train_odict = OrderedDict(
        (key, seq_odict[key]) for key in train_keys if key in seq_odict
    )
